    ...
    function kirimchat(event) {
      ...
      $('#USERNAME,#AVRfoto,#TEXTCHAT,#FILECHAT,#KLSS').not(':input[type=button],:input[type=submit], :input[type=reset],.file-path,.select-dropdown,.picker__select--year').each(function(index, item){
          if(nameArr.indexOf($(item).attr('name'))<0){
             nameArr.push({name: $(item).attr('name'), type: $(item).attr('type')});
          }
      });
      $('#USERNAME,#AVRfoto,#TEXTCHAT,#FILECHAT,#KLSS').not(':input[type=button],:input[type=submit], :input[type=reset],:input[type=file],.file-path,.select-dropdown,.picker__select--year').each(function(index, item){
